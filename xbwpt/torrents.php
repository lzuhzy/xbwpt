...
############################added 20101230##################################################################################
{
	if($wherecatina[0]%1000 ==0)
		$wherea[] = "category between $wherecatina[0] and $wherecatina[0]+10";
	else
		$wherea[] = "category = $wherecatina[0]";
...
<form method="get" name="searchbox" action="?">
	<table border="1" class="searchbox" cellspacing="0" cellpadding="5" width="100%">
		<tbody>
		<tr>
<?php if ($CURUSER['showsearchbox'] == 'no') { ?>
		<td class="colhead" align="center" colspan="2"><a href="javascript: klappe_news('searchboxmain')"><img class="plus" src="pic/trans.gif" id="picsearchboxmain" alt="Show/Hide" /><font size="2" color="red"><?php echo $lang_torrents['text_search_box'] ?></font></a></td>
<?php } else { ?>
		<td class="colhead" align="center" colspan="2"><a href="javascript: klappe_news('searchboxmain')"><img class="minus" src="pic/trans.gif" id="picsearchboxmain" alt="Show/Hide" /><font size="2" color="red"><?php echo $lang_torrents['text_search_box'] ?></font></a></td>
<?php } ?>
		</tr></tbody>
<?php if ($CURUSER['showsearchbox'] == 'no') { ?>
		...
		<tr>
			<td class="rowfollow" align="center">
				<table>
					<?php
#########################################################added 20101229#####################################################################
						function newcat($listarry)
						{
							$s .="<div id=\"navigation\">";
							$s .="<div id=\"catleft\">";
							$s .="<div id=\"bigcatname\"><a href=\"?cat=".$listarry[0]['id']."\" >".$listarry[0]['name']."</a></div>";
							$main_class = 1;
							for($i=1;$i<count($listarry);$i++)
							{	
								if($listarry[$i]['id']-$listarry[$i-1]['id'] != 1)
								{
									$main_class ++;
									$s .="<div id=\"bar\">&nbsp;</div>";
									if($main_class==7)
									...
									if ($main_class==count($listarry))
									{
										$s .="</div>";
									}
							
								}
								if(intval($listarry[$i]['id']/1000) == $main_class && $listarry[$i]['id']%1000 == 0)
								{
								    if ($main_class != "13") {//低质量分类不显示 Amanda add
									
									$s .="<div id=\"bigcatname\"><a href=\"?cat=".$listarry[$i]['id']."\" >".$listarry[$i]['name']."</a></div>";
								    }
								}
								else
								{
								    if ($listarry[$i]['id'] != "13001" && $listarry[$i]['id'] != '13002' && $listarry[$i]['id'] != '13003') { //不显示低质量分类 Amanda add
									    $s .= "<div class=\"soncatname\"><a href=\"?cat=".$listarry[$i]['id']."\" >".$listarry[$i]['name']."</a></div>";
								    }
								}
							}
							$s .="</div>";
			...
			<td class="rowfollow" align="center">
				<table>
					<tr>
						...
						<select class="med" name="incldead" style="width: 75px;">
...
if (!$Cache->get_page()){
	...
	$Cache->add_whole_row();
	if ($hotsearch)
	print("<tr><td class=\"embedded\" colspan=\"4\">&nbsp;&nbsp;".$hotsearch."</td></tr>");
	$Cache->end_whole_row();
	$Cache->cache_page();
}
echo $Cache->next_row();
