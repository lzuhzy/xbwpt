...
function usercpmenu ($selected = "home") {
	...
	print ("<li" . ($selected == "forum" ? " class=selected" : "") . "><a href=\"http://bbs.lzu.edu.cn/wForum/usermanagemenu.php\" target=\"_blank\">".$lang_usercp['text_forum_settings']."</a></li>");
  ...
  break;
		case "tracker":
			...
			function browsecheck($dbtable = "categories", $cbname = "cat"){
				...
				//Amanda add 2015-4-6
				if ($_POST["showsearchbox"] == 'yes')
                                        $showsearchbox = 'yes';
                                else $showsearchbox = 'no';
                                $updateset[] = "showsearchbox = " . sqlesc($showsearchbox);
...
if ($showaudiocodec) $audiocodecs = searchbox_item_list("audiocodecs");
			...
			if ($emailnotify_smtp=='yes' && $smtptype != 'none')
				...
				$categories = "<table>".($allowspecial ? "<tr><td class=embedded align=left><font class=big>".$lang_usercp['text_at_browse_page']."</font></td></tr></table><table>" : "")."<tr><td class=embedded align=left><b>";
				$categories .= ($brenablecatrow == false ? $brcatrow[0] : $lang_usercp['text_category'])."</b></td></tr><tr>";
		//	$i = 0;
...
tr_small($lang_usercp['row_funbox'],"<input type=checkbox name=showfb".($CURUSER["showfb"] == "yes" ? " checked" : "") ." value=yes>".$lang_usercp['text_show_funbox'] , 1);
		...
		<b>".$lang_usercp['text_show_searchbox'].": </b><br />"."<input type=checkbox name=showsearchbox ".($CURUSER['showsearchbox'] == 'yes' ? " checked" : "")." value=yes>".$lang_usercp['text_show_searchbox_description']."<br />
		...
		case "security":
			if ($type == 'confirm') {
				...
				if (!$oldpassword){
					...
					die;
				}elseif ($CURUSER["passhash"] != gen_pass($CURUSER["username"], $oldpassword)){
					stderr($lang_usercp['std_error'], $lang_usercp['std_wrong_password_note'].goback(), 0);
					#stderr($CURUSER["passhash"], $CURUSER["username"].$CURUSER['password'].gen_pass($CURUSER["username"], $CURUSER["password"]).goback(), 0);
					die;
				}else
				$updateset = array();
				$changedemail = 0;
