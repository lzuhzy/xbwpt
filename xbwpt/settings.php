...
if ($action == 'savesettings_main')	// save main
{
	...
	$validConfig = array('site_online','max_torrent_size','announce_interval', 'annintertwoage', 'annintertwo', 'anninterthreeage', 'anninterthree', 'signup_timeout','minoffervotes','offervotetimeout','offeruptimeout','maxsubsize','postsperpage', 'topicsperpage', 'torrentsperpage', 'maxnewsnum','max_dead_torrent_time','maxusers','torrent_dir', 'iniupload','iniseedbonus','SITEEMAIL', 'ACCOUNTANTID', 'ALIPAYACCOUNT', 'PAYPALACCOUNT', 'SLOGAN', 'icplicense', 'autoclean_interval_one', 'autoclean_interval_two', 'autoclean_interval_three','autoclean_interval_four', 'autoclean_interval_five','reportemail','invitesystem','loginadd','enablerequest','registration','showhotmovies','showclassicmovies','showimdbinfo', 'enablenfo', 'enableschool','restrictemail','showpolls','showstats','showlastxtorrents', 'showtrackerload','showshoutbox','showfunbox','showoffer','sptime','showhelpbox','enablebitbucket', 'smalldescription','altname','extforum','extforumurl','defaultlang','defstylesheet', 'donation','spsct','browsecat','specialcat','waitsystem','maxdlsystem','bitbucket','torrentnameprefix', 'showforumstats','verification','invite_count','invite_timeout', 'seeding_leeching_time_calc_start','startsubid', 'logo');
	GetVar($validConfig);
	unset($MAIN);
...
elseif ($action == 'bonussettings'){
	...
	tr($lang_settings['row_ten_gb_credit'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=tengbupload value='".(isset($BONUS["tengbupload"]) ? $BONUS["tengbupload"] : 1300 )."'>".$lang_settings['text_ten_gb_credit_note'], 1);
	tr($lang_settings['row_fifty_gb_credit'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=fiftygbupload value='".(isset($BONUS["fiftygbupload"]) ? $BONUS["fiftygbupload"] : 7000 )."'>".$lang_settings['text_fifty_gb_credit_note'], 1);
	tr($lang_settings['row_onehundred_gb_credit'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=onehundredgbupload value='".(isset($BONUS["onehundredgbupload"]) ? $BONUS["onehundredgbupload"] : 10300 )."'>".$lang_settings['text_onehundred_gb_credit_note'], 1);
...
elseif ($action == 'mainsettings')	// main settings
{
	...
	yesorno($lang_settings['row_enable_enablerequest'], 'enablerequest', $MAIN['enablerequest'], $lang_settings['row_allow_enablerequest']);
