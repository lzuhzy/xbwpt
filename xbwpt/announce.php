...
//$fp = fopen("1.txt","a");
//fwrite($fp,$agent."B".$clicheck_res."\n");
...
/* Amanda add 2012-1-27
$resip = sql_query("SELECT id FROM peers where userid = ".$userid." AND (ip != '".$ip."' OR port != ".$port." OR agent != '".$agent."')");
	if(mysql_num_rows($resip) > 0)
	{
		err("Sorry, the account could only be used privately! ............");
		die();
	}
*/
