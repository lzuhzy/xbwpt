...
//marked 2010-12-29 规范种子命名。原始命令窗口		
...
$s = "<select name=\"type2\" id=\"browsecat\" onchange=\"changetype();\" onkeypress=\"changetype();\" ".($allowtwosec ? $disablespecial : "").">\n<option value=\"0\">".$lang_upload['select_choose_one']."</option>\n";
$cats = genrelist($browsecatmode);															
$getlangfoldercookieblabla = get_langfolder_cookie();
foreach ($cats as $row)
{
if($row["id"]%1000 == 0)	
$s .= "<option value=\"" . $row["id"] . "\">" . htmlspecialchars($row["name". ($getlangfoldercookieblabla == 'chs'? '_chs':'')]) . "</option>\n";
}
$s .= "</select>\n";
}
else $s ="";
tr($lang_upload['row_type']."<font color=\"red\">*</font>", ($allowtwosec ? $lang_upload['text_to_browse_section'] : "").$s.($allowtwosec ? $lang_upload['text_to_special_section'] : "").$s2.($allowtwosec ? $lang_upload['text_type_note'] : "")."rmvb、avi、rm格式以及TS版等低质量种子请选择“低质量”分类，否则管理员可直接删除种子",1);
		$arr  = array();
				disTorrentNameInput($arr);
##################################################added end#####################
				if ($smalldescription_main == 'yes')
				tr($lang_upload['row_small_description'], "<input type=\"text\" style=\"width: 650px;\" name=\"small_descr\" /><br /><font class=\"medium\">".$lang_upload['text_small_description_note']."</font>", 1);
