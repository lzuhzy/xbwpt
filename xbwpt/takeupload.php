...
$type = $_POST["type"];
$i = 0;
$torrentname_ary=array();
foreach(explode(":","cat01:cat02:cat03:cat04:cat05:cat06:cat07:cat08:cat09:cat10:cat11:cat12:cat13") as $v)
{
	$ary_tem = array($i => $_POST[$v]);
	$i ++;
	$torrentname_ary = array_merge($torrentname_ary,$ary_tem);
}
$type_main = array('1000'=>'电影','2000'=>'剧集','3000'=>'音乐','4000'=>'动漫','5000'=>'游戏','6000'=>'综艺','7000'=>'体育','8000'=>'软件','9000'=>'学习','10000'=>'其他','11000'=>'纪录片','12000'=>'高清','13000'=>'低质量');

//保证命名正确填写 added by longth 2011.2.23
if(in_array('', $torrentname_ary[$type2/1000-1]))
	bark("种子命名表单未填写完整，请认真填写。...PS:鼠标移动到输入框内会出现填写说明:-)");
//added end


$t_name = "[".$type_main[$type2]."][".implode("][",$torrentname_ary[$type2/1000-1])."]";
$save_name = $t_name;
#########################################added end####################
...
$ret = sql_query("INSERT INTO torrents (filename, owner, visible, anonymous, name, size, numfiles, type, dburl, url, small_descr, descr, ori_descr, category, source, medium, codec, audiocodec, standard, processing, team, save_as, sp_state, added, last_action, nfo, info_hash) VALUES (".sqlesc($t_name).", ".sqlesc($CURUSER["id"]).", 'yes', ".sqlesc($anonymous).", ".sqlesc($torrent).", ".sqlesc($totallen).", ".count($filelist).", ".sqlesc($type).", ".sqlesc($dburl).", ".sqlesc($url).", ".sqlesc($small_descr).", ".sqlesc($descr).", ".sqlesc($descr).", ".sqlesc($catid).", ".sqlesc($sourceid).", ".sqlesc($mediumid).", ".sqlesc($codecid).", ".sqlesc($audiocodecid).", ".sqlesc($standardid).", ".sqlesc($processingid).", ".sqlesc($teamid).", ".sqlesc($save_name).", ".sqlesc($sp_state) .
