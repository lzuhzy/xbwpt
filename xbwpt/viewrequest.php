		...
		case "list" :
			{
				...
				if ($rownumber [0] == 0)
					...
					print ("<h1 align=center><a href=viewrequest.php?action=new><input type=\"button\" value=\"" . $lang_req['head_req'] . "\" onclick=\"window.location='viewrequest.php?action=new';\" style=\"font-weight: bold\"/></a> <a href=rules.php><input type=\"button\" value=\"规则\" onclick=\"rules.php';\" style=\"color: red; font-weight: bold\"/></a></h1>") ;
					print ($pagertop) ;
					print ("<table width=940 border=1 cellspacing=0 cellpadding=5 style=border-collapse:collapse >\n") ;
					if (get_user_class () >= 13) {
						print ("<form action=\"viewrequest.php\" method=\"GET\" onsubmit=\"return confirm('确认操作？');\">\n") ;
		...
		case "view" :
			{
				if (is_numeric ( $_GET ["id"] )) {
					...
					if ($arr ["finish"] == "no")
						tr ( "追加悬赏", "<form action=viewrequest.php method=post> <input type=hidden name=action value=addamount><input type=hidden name=reqid value=" . $arr ["id"] . "><input size=6 name=amount value=100 ><input type=submit value=提交 > 追加悬赏每次将扣减5个魔力值作为手续费" . $amountadder . "</form>", 1 );
...
function quick_reply_to(username)
{
		...
		case "new" : // 新增求种
			{
				if (get_user_class () >= 2) {
					...
					tr ( "悬赏：", "<input name=amount size=11 value=100>赏金不得低于100魔力值，每次求种将扣去10魔力值作为手续费。<br/>", 1 );
					print ("<tr><td class=rowhead align=right valign=top><b>介绍：</b></td><td class=rowfollow align=left>") ;
					textbbcode ( "edit", "introduce", $arr ["introduce"] );
					print ("</td></tr>") ;
		...
		case "takeadded" :
			{
				...
				$amount = $_POST ["amount"];
				if ($amount < 100)
					stderr ( "出错了！", "发布求种赏金不得小于100个魔力值！<a href=javascript:history.go(-1)>点击这里返回</a>", 0 );
				if ($amount > 5000)
					stderr ( "出错了！", "发布求种赏金不得大于5000个魔力值！<a href=javascript:history.go(-1)>点击这里返回</a>", 0 );
				$amount += 10;
				if ($amount + 10 > $CURUSER [seedbonus])
				...
				if (get_user_class () >= 2) {
					...
					writeBonusComment($CURUSER [id],"使用$amount 魔力值新增了求种 " . mysql_insert_id (). sqlesc ( $_POST ["name"],$add ));
	...
	<input type=hidden name=reqid value=\"" . $_GET ["id"] . "\" />
	请输入种子的id:（请根据地址栏填写）<br/>http://pt.xbwbbs.com/details?id=<input type=text name=torrentid size=11/><br/>
		...
		case "addamount" :
			{
				...
				writeBonusComment($CURUSER [id],"使用$newseedbonus 魔力值追加了悬赏". sqlesc ( $_POST ["reqid"],$add ));
					...
					if (! $_GET ['job'] || $_GET ['job'] == '删除') {
						...
						if (mysql_num_rows ( $amountadds ) > 0)
							while ( $amountadd = mysql_fetch_array ( $amountadds ) ) {
								...
								writeBonusComment($amountadd ["bonusfromuserid"],"求种$id 被撤销，返还$amountadd[bonus] 魔力值",$add);
						...
						writeBonusComment($arr ["userid"],"求种$id 被撤销，返还$amount 魔力值",$add);
						sql_query ( "DELETE FROM givebonus WHERE bonustotorrentid ='" . $id . "' AND type = '3'" ) or sqlerr ( __FILE__, __LINE__ );
		...
		case "confirm" :
			{
				...
				if ($arr ['userid'] == $CURUSER [id] || get_user_class () >= 13) {
					...
					foreach ( $owner as $id ) {
						...
						writeBonusComment($id,"求种$_POST[id] 被确认，增加悬赏$amount 魔力值",$add);
