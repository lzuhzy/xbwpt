...
function maketable($res, $mode = 'seeding')
{
	...
	$ret = "<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" width=\"98%\">
		<tr>".("<td class=\"colhead\" align=\"center\" style=\"padding: 0px\">".$lang_getusertorrentlistajax['col_type']."</td>
		<td class=\"colhead\" align=\"center\">".$lang_getusertorrentlistajax['col_name']."</td>")
		...
		.($showcomnum ? "<td class=\"colhead\" align=\"center\"><img class=\"snatched\" src=\"pic/trans.gif\" alt=\"snatched\" title=\"完成数\" /></td>" : "")
		...
		.($showanonymous ? "<td class=\"colhead\" align=\"center\">".$lang_getusertorrentlistajax['col_anonymous']."</td>" : "")."</tr>\n";
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		$ret .= "<tr" .  $sphighlight  . ">".("<td class=\"rowfollow nowrap\" align=\"center\" style='padding: 0px'>".return_category_image($arr['category'])."</td>\n" .
		"<td class=\"rowfollow\" width=\"500px\" align=\"left\"  style='padding-top:0;padding-bottom: 0px;'><a href=\"".htmlspecialchars("details.php?id=".$arr[torrent]."&hit=1")."\" title=\"".$nametitle."\"><b>" . $dispname . "</b></a>". $sp_torrent .($dissmall_descr == "" ? "" : "<br />" . $dissmall_descr) . "</td>");
		//size
		if ($showsize)
		...
		if ($showuploaded){
			...
			$ret .= "<td class=\"rowfollow\"  width=\"80px\" align=\"center\">".$imuploaded.'<br />'.$uploaded."</td>";
		}
		//downloaded amount
		if ($showdownloaded){
			$downloaded = mksize($arr["downloaded"]);
			$imdownloaded = mksize($arr["imdownloaded"]);
			$ret .= "<td class=\"rowfollow\" width=\"90px\" align=\"center\">".$imdownloaded.'<br />'.$downloaded."</td>";
		}
		//ratio
		if ($showratio){
			if ($arr['downloaded'] > 0)
...
print("\n<h1 align=center>".get_plain_username($id)."  $typestate (共 $count)</h1>");
