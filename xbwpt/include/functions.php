...
function menu ($selected = "home") {
	...
	if (preg_match("/index/i", $script_name)) {
		...
		$selected = "torrents";
	}elseif (preg_match("/poor/i", $script_name)) {
		$selected = "poor";
...
##################控制面板移动到下面###############
	...
	#################暂时注销掉了#####################
	print ("<li" . ($selected == "topten" ? " class=\"selected\"" : "") . "><a href=\"topten.php?type=2\">".$lang_functions['text_top_ten']."</a></li>");
...
<script type="text/javascript" src="login.js<?php echo  $cssupdatedate?>"></script>
<script type="text/javascript" src="menu.js<?php echo  $cssupdatedate?>"></script>
...
{
?>
			<div class="logo"><a href="index.php"><?php echo htmlspecialchars($SITENAME)?></a></div>
...
<?php if (!$CURUSER) { ?>
<a href="login.php"><font class="big"><b><?php echo $lang_functions['text_login'] ?></b></font></a> | <a href="http://bbs.lzu.edu.cn/bbsreg.html" target="_blank"><font class="big"><b><?php echo $lang_functions['text_signup'] ?></b></font></a> | <a href="http://bbs.lzu.edu.cn/nForum/board/Pt" target="_blank"><font class="big" color=red><b><?php echo $lang_functions['text_pt'] ?></b></font></a> | <a href="/rules.php"><font class="big"><b><?php echo $lang_functions['text_rule'] ?></b></font></a> | <a href="/faq.php"><font class="big"><b><?php echo $lang_functions['text_faq'] ?></b></font></a> | <a href="/staff.php"><font class="big"><b><?php echo $lang_functions['text_admin'] ?></b></font></a>
<?php } 
else {
	begin_main_frame();
	menu ();
...
print("<a href=\"cheaterbox.php\"><img class=\"cheaterbox\" alt=\"cheaterbox\" src=\"pic/trans.gif\" />".$lang_functions['title_cheaterbox']."</a>(".$totalcheaters.")	<a href=\"reports.php\"><img class=\"reportbox\" alt=\"reportbox \" src=\"pic/trans.gif\" />".$lang_functions['title_reportbox']."</a> (".$totalreports.")	<a href=\"staffbox.php\"><img class=\"staffbox\" alt=\"staffbox\" src=\"pic/trans.gif\" />".$lang_functions['title_staffbox']."</a>(".$totalsm.")");
}
?>
<br />
...
function genrelist($catmode = 1) {
	...
	if (!$ret = $Cache->get_value('category_list_mode_'.$catmode)){
		$ret = array();
		$res = sql_query("SELECT id, mode, name, name_chs, image FROM categories WHERE mode = ".sqlesc($catmode)." ORDER BY sort_index, id");
		while ($row = mysql_fetch_array($res))
			$ret[] = $row;
...
while ($row = mysql_fetch_assoc($res))
{
	...
	print("<tr class=" .$class . $sphighlight . ">\n");
	$i ++;
			...
			if ($showextinfo['imdb'] == 'yes' && $row["url"])
{
	...
	print("<td class=\"rowfollow\" width=\"100%\" align=\"left\"><table class=\"torrentname\" width=\"100%\"><tr class=\"".$class . $sphighlight . "\"><td class=\"embedded\">".$stickyicon."<a $short_torrent_name_alt $mouseovertorrent href=\"details.php?id=".$id."&amp;hit=1\"><b>".htmlspecialchars($dispname)."</b></a>");
	...
	if (get_user_class() >= $torrentmanage_class)
	{
		print("<td class=\"rowfollow\"><a href=\"".htmlspecialchars("fastdelete.php?id=".$row[id])."\">".$lang_functions['text_delete']."</a>");
		print("<br /><a href=\"edit.php?returnto=" . rawurlencode($_SERVER["REQUEST_URI"]) . "&amp;id=" . $row["id"] . "\"><font color=blue>".$lang_functions['text_edit']."</font></a></td>\n");
	}
	print("</tr>\n");
	$counter++;
}
...
function return_category_image($categoryid, $link="")
{
	...
	if ($catImg[$categoryid]) {
		...
		$arry_c = explode("-", $categoryrow["name_chs"]);

		$catImg[$categoryid] = $catimg = "&nbsp【".$arry_c[0]."】&nbsp</a><br><a href=\"?cat=".$categoryrow['id']."\">&nbsp[".$arry_c[1]."]&nbsp";
	$id1 = intval($categoryid/1000)*1000;
	}
	if ($link) {
		$catimg = "<a href=\"".$link."cat=" . $id1 . "\">".$catimg."</a>";
	}
	return $catimg;
}
//用于添加魔力值记录
...
function userccss()
{
	...
	if (!$row = $Cache->get_value('user_'.$CURUSER["id"].'_css')){
                ...
                $res = sql_query('SELECT css FROM  usercss WHERE  userid  ='.sqlesc($CURUSER["id"]).' LIMIT 1 ');
                $row = "<style type='text/css'>".$res['css']."</style>";
...
{
	$magic = 'wwj&kcn4SMTHBBS MD5 p9w2d gen2rat8, //grin~~, 2001/5/7';
	$passhash = md5($magic . $passwd . $magic. $username);
	return $passhash;
}

//规范命名方法----种子自动准确命名。
function disTorrentNameInput($arrValues){
	...
	//cat01电影
	//[2007][怪物史莱克3][Shrek.The.Third][主演][动画][英文字幕][DVDRip]

	$s.="\n".'<tr><td class="rowhead nowrap" valign="top" align="right">种子标题<font color=red>*</font></td>'."\n";
	$s.='<td>'."\n";
	$s.='<table id="cat01" class="category" style="display:none">';
	$s .= '<td>类型   <select name="type[]">
					<option value="1001" >中国</option>
					<option value="1003" >日韩</option>
					<option value="1002" >欧美</option>
					<option value="1004" >其他国家</option>
					<option value="1005" >高清</option>
					</select></td>'."\n";
	$s .='<td>年份   <input type="text" name="cat01[]" title="电影上映的年份" style="width:40px;" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>中文名 <input type="text" name="cat01[]" title="电影中文名称" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>英文名 <input type="text" name="cat01[]" title="电影英文名称" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>主演   <input type="text" name="cat01[]" title="主演，中英文名均可" value="'.$v[3].'" /></td>'."\n";
	$s.='<td>类型   <input type="text" name="cat01[]" title="电影类型：恐怖？科幻？喜剧？爱情？动作？" value="'.$v[4].'" /></td>'."\n";
	$s.='<td>字幕   <select name="cat01[]">
					<option value="暂无字幕" '.$sel('暂无字幕',$v[5]).'>暂无字幕</option>
					<option value="英文字幕" '.$sel('英文字幕',$v[5]).'>英文字幕</option>
					<option value="中文字幕" '.$sel('中文字幕',$v[5]).'>中文字幕</option>
					<option value="中英字幕" '.$sel('中英字幕',$v[5]).'>中英字幕</option>
					<option value="帖内中文字幕" '.$sel('帖内中文字幕',$v[5]).'>帖内中文字幕</option>
					<option value="帖内中英字幕" '.$sel('帖内中英字幕',$v[5]).'>帖内中英字幕</option>
					<option value="无需字幕" '.$sel('',$v[5]).'>无需字幕</option>
					</select></td>'."\n";
	$s.='<td>格式   <select name="cat01[]" title="电影格式，具体含义请参看电影版规则" >
					<option value="1080P"'.$sel('1080P',$v[6]).'>1080P</option>
					<option value="720P"'.$sel('720P',$v[6]).'>720P</option>
					<option value="HD-DVDRip"'.$sel('HD-DVDRip',$v[6]).'>HD-DVDRip</option>
					<option value="HR-HDTV"'.$sel('HR-HDTV',$v[6]).'>HR-HDTV</option>
					<option value="BDRip"'.$sel('BDRip',$v[6]).'>BDRip</option>
					<option value="HDTVRip"'.$sel('HDTVRip',$v[6]).'>HDTVRip</option>
					<option value="DVDRip"'.$sel('DVDRip',$v[6]).'>DVDRip</option>
					<option value="R5"'.$sel('R5',$v[6]).'>R5</option>
					<option value="DVDSCR"'.$sel('DVDSCR',$v[6]).'>DVDSCR</option>
					<option value="halfCD"'.$sel('halfCD',$v[6]).'>halfCD</option>
					<option value="MiniSD"'.$sel('MiniSD',$v[6]).'>MiniSD</option>
					<option value="MP3"'.$sel('MP3',$v[6]).'>MP3</option>
					<option value="APE"'.$sel('APE',$v[6]).'>APE</option>
					<option value="其他"'.$sel('',$v[6]).'>其他</option>
				    </select></td>'."\n";
	$s.='</table>'."\n";


	//cat02 剧集
	//[大陆][幸福像花儿一样][上部][孙俪,邓超,辛柏青,殷桃][原著石钟山][rmvb]
	$s.='<table id="cat02" class="category" style="display:none">';
	$s.= '<td>类型   <select name="type[]">
					<option value="2001" >大陆</option>
					<option value="2002" >港台</option>
					<option value="2003" >日剧</option>
					<option value="2004" >韩剧</option>
					<option value="2005" >美剧</option>
					<option value="2006" >其他</option>
					</select></td>'."\n";
	$s.='<td>中文名 <input type="text" name="cat02[]" title="电视剧中文名称" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>英文名 <input type="text" name="cat02[]" title="美剧请输入英文名" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>集数/集别   <input type="text" name="cat02[]" title="第1集？40集？" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>主演   <input type="text" name="cat02[]" title="主演" value="'.$v[3].'" /></td>'."\n";
	$s.='<td>字幕   <select name="cat02[]">
					<option value="暂无字幕" '.$sel('暂无字幕',$v[4]).'>暂无字幕</option>
					<option value="英文字幕" '.$sel('英文字幕',$v[4]).'>英文字幕</option>
					<option value="中文字幕" '.$sel('中文字幕',$v[4]).'>中文字幕</option>
					<option value="无须字幕" '.$sel('',$v[4]).'>无需字幕</option>
					</select></td>'."\n";
	$s.='<td>格式   <select name="cat02[]">
					<option value="1080P"'.$sel('1080P',$v[5]).'>1080P</option>
					<option value="720P"'.$sel('720P',$v[5]).'>720P</option>
					<option value="DVDRip"'.$sel('DVDRip',$v[5]).'>DVDRip</option>
					<option value="HalfCD"'.$sel('HalfCD',$v[5]).'>HalfCD</option>
					<option value="HDTV"'.$sel('HDTV',$v[5]).'>HDTV</option>
					<option value="DSR"'.$sel('DSR',$v[5]).'>DSR</option>
					<option value="D2R"'.$sel('D2R',$v[5]).'>DVD2RMVB</option>
					<option value="V2R"'.$sel('V2R',$v[5]).'>VCD2RMVB</option>
					<option value="RMVB"'.$sel('RMVB',$v[5]).'>RMVB</option>
					<option value="TVRip"'.$sel('TVRip',$v[5]).'>TVRip</option>
					<option value="其他"'.$sel('',$v[5]).'>其他</option>
					</select></td>'."\n";
	$s.='</table>'."\n";

	//国内音乐 [年代][歌手][专辑][格式][码率]
	$s.='<table id="cat03" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="3001" >华语</option>
					<option value="3002" >日韩</option>
					<option value="3003" >欧美</option>
					<option value="3004" >古典</option>
					<option value="3005" >MV</option>
					<option value="3006" >无损</option>
					</select></td>'."\n";
	$s.='<td>发行日期 <input type="text" name="cat03[]" title="不知道就空着吧" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>歌手     <input type="text" name="cat03[]" title="歌手名称" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>专辑     <input type="text" name="cat03[]" title="专辑名称" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>格式     <select name="cat03[]">
						<option value="MP3"'.$sel('MP3',$v[3]).'>MP3</option>
						<option value="WMA"'.$sel('WMA',$v[3]).'>WMA</option>
						<option value="WAV"'.$sel('WAV',$v[3]).'>WAV</option>
						<option value="APE"'.$sel('APE',$v[3]).'>APE</option>
						<option value="BIN"'.$sel('BIN',$v[3]).'>BIN</option>
						<option value="VOB"'.$sel('VOB',$v[3]).'>VOB</option>
						<option value="FLAC"'.$sel('FLAC',$v[3]).'>FLAC</option>
						<option value="OGG"'.$sel('OGG',$v[3]).'>OGG</option>
						<option value="NoathVBR"'.$sel('NoathVBR',$v[3]).'>NoathVBR</option>
						<option value="WV"'.$sel('WV',$v[3]).'>WV</option>
						<option value="MID"'.$sel('CBR',$v[3]).'>MID</option>
						<option value="MKV"'.$sel('MKV',$v[3]).'>MKV</option>
						<option value="DVDRip"'.$sel('DVDRip',$v[3]).'>DVDRip</option>
						<option value="MPG"'.$sel('MPG',$v[3]).'>MPG</option>
						<option value="AVI"'.$sel('AVI',$v[3]).'>AVI</option>
						<option value="AAC"'.$sel('AAC',$v[3]).'>AAC</option>
						<option value="DAT"'.$sel('DAT',$v[3]).'>DAT</option>
						<option value="TAK"'.$sel('TAK',$v[3]).'>TAK</option>						
						<option value="WMV"'.$sel('WMV',$v[3]).'>WMV</option>
						<option value="RMVB"'.$sel('RMVB',$v[3]).'>RMVB</option>
						<option value="其他"'.$sel('',$v[3]).'>其他</option>
					  </select></td>'."\n";
	$s.='<td>码率     <select name="cat03[]" title="不知道什么是码率请选择其他">
						<option value="128kbps"'.$sel('128kbps',$v[4]).'>128kbps</option>
						<option value="192kbps"'.$sel('192kbps',$v[4]).'>192kbps</option>
						<option value="192k VBR"'.$sel('192k VBR',$v[4]).'>192k VBR</option>
						<option value="256kbps"'.$sel('256kbps',$v[4]).'>256kbps</option>
						<option value="320kbps"'.$sel('320kbps',$v[4]).'>320kbps</option>
						<option value="Noath VBR"'.$sel('Noath VBR',$v[4]).'>Noath VBR</option>
						<option value="其他"'.$sel('',$v[4]).'>其他</option>
						</select></td>'."\n";
	$s.='</table>'."\n";
	
	
	//cat04
	//[动画/漫画/动漫音乐/周边][字幕组，命名][集数/全集][AVI/rm/rmvb/mp3/其他]
	///字幕组；名称；集数；相关属性；格式。


	$s.='<table id="cat04" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="4001" >动画</option>
					<option value="4002" >漫画</option>
					<option value="4003" >音乐</option>
					<option value="4004" >合集</option>
					</select></td>'."\n";
	$s.='<td>字幕组  <input type="text" name="cat04[]" title="字幕组" class="w100" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>英文或罗马音名称 <input type="text" name="cat04[]" title="命名" class="w100" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>中文名称 <input type="text" name="cat04[]" title="命名" class="w100" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>集数    <input type="text" name="cat04[]" title="集数" class="w100" value="'.$v[3].'" /></td>'."\n";
	$s.='<td>字幕 <select name="cat04[]">
								<option value="简体中文"'.$sel('简体中文',$v[4]).'>简体中文</option>
								<option value="繁体中文"'.$sel('繁体中文',$v[4]).'>繁体中文</option>
								<option value="其他"'.$sel('',$v[5]).'>其他</option>
							</select></td>'."\n";
	$s.='<td>格式           <select name="cat04[]">
								<option value="MKV"'.$sel('MKV',$v[5]).'>MKV</option>
								<option value="APE"'.$sel('APE',$v[5]).'>APE</option>
								<option value="RAR"'.$sel('RAR',$v[5]).'>RAR</option>
								<option value="ZIP"'.$sel('ZIP',$v[5]).'>ZIP</option>
								<option value="AVI"'.$sel('AVI',$v[5]).'>AVI</option>
								<option value="MP4"'.$sel('MP4',$v[5]).'>MP4</option>
								<option value="WMV"'.$sel('WMV',$v[5]).'>WMV</option>
								<option value="ISO"'.$sel('ISO',$v[5]).'>ISO</option>
								<option value="RM"'.$sel('RM',$v[5]).'>RM</option>
								<option value="RMVB"'.$sel('RMVB',$v[5]).'>RMVB</option>
								<option value="MP3"'.$sel('MP3',$v[5]).'>MP3</option>
								<option value="其他"'.$sel('',$v[5]).'>其他</option>
							</select></td>'."\n";
	$s.='</table>'."\n";
	
	//cat05 游戏
	//[实况足球9欧洲版][Pro.Evolution.Soccer.5][Sports][正式英文DVD版]
	$s.='<table id="cat05" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="5001" >PC</option>
					<option value="5002" >PSP</option>
					<option value="5003" >其他</option>
					</select></td>'."\n";
	$s.='<td>平台 <select name="cat05[]">
					<option value="PC"'.$sel('PC',$v[0]).'>PC</option>
					<option value="PS"'.$sel('PS',$v[0]).'>PS</option>
					<option value="PS2"'.$sel('PS2',$v[0]).'>PS2</option>
					<option value="PS3"'.$sel('PS3',$v[0]).'>PS3</option>
					<option value="XBOX"'.$sel('XBOX',$v[0]).'>XBOX</option>
					<option value="XBOX360"'.$sel('XBOX360',$v[0]).'>XBOX360</option>
					<option value="NGC"'.$sel('NGC',$v[0]).'>NGC</option>
					<option value="wii"'.$sel('wii',$v[0]).'>wii</option>
					<option value="NDS/L"'.$sel('NDS/L',$v[0]).'>NDS/L</option>
					<option value="GBA"'.$sel('GBA',$v[0]).'>GBA</option>
					<option value="PSP"'.$sel('PSP',$v[0]).'>PSP</option>
					<option value="DC"'.$sel('DC',$v[0]).'>DC</option>
					<option value="视频"'.$sel('视频',$v[0]).'>视频</option>
					<option value="音乐"'.$sel('音乐',$v[0]).'>音乐</option>
					<option value="FC模拟器"'.$sel('FC模拟器',$v[0]).'>FC模拟器</option>
					<option value="街机模拟器"'.$sel('街机模拟器',$v[0]).'>街机模拟器</option>
					<option value="其他"'.$sel('',$v[0]).'>其他</option>
				</select></td>'."\n";
	$s.='<td>英文名 <input type="text" name="cat05[]" title="最好是按照官方的版本大小写"  value="'.$v[1].'"  /></td>'."\n";
	$s.='<td>中文名 <input type="text" name="cat05[]" title="国内有代理的话，用国内代理翻译的中文名，没有的话尽量按照主流翻译。而类似FIFA，NBA Live之类的游戏不需要中文名。"  value="'.$v[2].'" /></td>'."\n";
	$s.='<td>类型 <select name="cat05[]">
					<option value="赛车游戏"'.$sel('赛车游戏',$v[3]).'>赛车游戏</option>
					<option value="格斗游戏"'.$sel('格斗游戏',$v[3]).'>格斗游戏</option>
					<option value="体育游戏"'.$sel('体育游戏',$v[3]).'>体育游戏</option>
					<option value="战略游戏"'.$sel('战略游戏',$v[3]).'>战略游戏</option>
					<option value="角色扮演"'.$sel('角色扮演',$v[3]).'>角色扮演</option>
					<option value="动作游戏"'.$sel('动作游戏',$v[3]).'>动作游戏</option>
					<option value="冒险游戏"'.$sel('冒险游戏',$v[3]).'>冒险游戏</option>
					<option value="射击游戏"'.$sel('射击游戏',$v[3]).'>射击游戏</option>
					<option value="桌面游戏"'.$sel('桌面游戏',$v[3]).'>桌面游戏</option>
					<option value="益智游戏"'.$sel('益智游戏',$v[3]).'>益智游戏</option>
					<option value="养成游戏"'.$sel('养成游戏',$v[3]).'>养成游戏</option>
					<option value="卡片游戏"'.$sel('卡片游戏',$v[3]).'>卡片游戏</option>
					<option value="即时战略游戏"'.$sel('即时战略游戏',$v[3]).'>即时战略游戏</option>
					<option value="第一人称射击游戏"'.$sel('第一人称射击游戏',$v[3]).'>第一人称射击游戏</option>
					<option value="动作角色扮演游戏"'.$sel('动作角色扮演游戏',$v[3]).'>动作角色扮演游戏</option>
					<option value="战略角色扮演游戏"'.$sel('战略角色扮演游戏',$v[3]).'>战略角色扮演游戏</option>
					<option value="在线角色扮演"'.$sel('在线角色扮演',$v[3]).'>在线角色扮演</option>
					<option value="音乐游戏"'.$sel('音乐游戏',$v[3]).'>音乐游戏</option>					
					<option value="文字AVG"'.$sel('文字AVG',$v[3]).'>文字AVG</option>					
					<option value="模拟游戏"'.$sel('模拟游戏',$v[3]).'>模拟游戏</option>					
					<option value="其他种类游戏"'.$sel('',$v[3]).'>其他种类游戏</option>
					</select></td>'."\n";
	$s.='<td>制作公司 <input type="text" name="cat05[]" title="可选，知道的尽量写上" value="'.$v[4].'" /></td>'."\n";
	$s.='<td>数据格式 <input type="text" name="cat05[]" title="尽量写。主流的游戏安装大多是CD镜像，DVD镜像，还有EXE，还有RAR压缩的，以及免安装的。其他的可以不填写。镜像的写清镜像数量，比如CD*2。" value="'.$v[5].'" /></td>'."\n";
	$s.='<td>语言     <select name="cat05[]">
						<option value="简体中文"'.$sel('简体中文',$v[6]).'>简体中文</option>
						<option value="繁体中文"'.$sel('繁体中文',$v[6]).'>繁体中文</option>						
						<option value="英文"'.$sel('英文',$v[6]).'>英文</option>
						<option value="日文"'.$sel('日文',$v[6]).'>日文</option>
						<option value="其他"'.$sel('',$v[6]).'>其他</option>
					  </select></td>'."\n";
	...
	//cat06
	//综艺 [节目地区(备选)][节目日期][节目名称][节目内容]
	$s.='<table id="cat06" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="6001" >大陆</option>
					<option value="6002" >港台</option>
					<option value="6003" >日韩</option>
					<option value="6004" >其他</option>
					</select></td>'."\n";
	$s.='<td>节目日期 <input type="text" name="cat06[]" title="比如2008-03-31"  value="'.$v[0].'" /></td>'."\n";
	$s.='<td>节目名称 <input type="text" name="cat06[]" title="综艺节目名称" class="w150" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>节目内容 <input type="text" name="cat06[]" title="内容简单介绍，比如主持人，嘉宾，访谈人物，专题内容等" class="w150" value="'.$v[2].'" /></td>'."\n";
	$s.='</tr></table>'."\n";
	$s.='</div>'."\n";

	//体育
	$s.='<table id="cat07" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="7001" >足球</option>
					<option value="7002" >篮球</option>
					<option value="7003" >其他</option>
					</select></td>'."\n";
	$s.='<td>节目日期 <input type="text" name="cat07[]" value="'.$v[0].'" title="比如2008-03-31,没有日期或日期不清的可不写" /></td>'."\n";
	$s.='<td>节目名称 <input type="text" name="cat07[]" value="'.$v[1].'" title="节目名称" class="w150" /></td>'."\n";
	$s.='<td>解说/语言 <input type="text" name="cat07[]" value="'.$v[2].'" title="解说/语言" class="w150" /></td>'."\n";
	$s.='<td>格式     <select name="cat07[]">
						<option value="720p"'.$sel('720p',$v[3]).'>720p</option>
						<option value="MKV"'.$sel('MKV',$v[3]).'>MKV</option>
						<option value="AVI"'.$sel('AVI',$v[3]).'>AVI</option>
						<option value="RMVB"'.$sel('RMVB',$v[3]).'>RMVB</option>
						<option value="WMV"'.$sel('WMV',$v[3]).'>WMV</option>
						<option value="MP4"'.$sel('MP4',$v[3]).'>MP4</option>
						<option value="MPG"'.$sel('MPG',$v[3]).'>MPG</option>
						<option value="FLV"'.$sel('FLV',$v[3]).'>FLV</option>
						<option value="其他"'.$sel('',$v[3]).'>其他</option>
					  </select></td>'."\n";
	$s.='</table>'."\n";
	...
	//cat08
	//软件 [名称][版本][分类][格式]
	$s.='<table  id="cat08" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="8001" >镜像</option>
					<option value="8002" >其他</option>
					</select></td>'."\n";
	$s.='<td>软件类型 <select name="cat08[]">
					<option value="系统工具"'.$sel('系统工具',$v[0]).'>系统工具</option>
					<option value="安全工具"'.$sel('安全工具',$v[0]).'>安全工具</option>
					<option value="网络工具"'.$sel('网络工具',$v[0]).'>网络工具</option>
					<option value="媒体工具"'.$sel('媒体工具',$v[0]).'>媒体工具</option>
					<option value="图形图像"'.$sel('图形图像',$v[0]).'>图形图像</option>
					<option value="磁盘工具"'.$sel('磁盘工具',$v[0]).'>磁盘工具</option>
					<option value="办公软件"'.$sel('办公软件',$v[0]).'>办公软件</option>
					<option value="游戏娱乐"'.$sel('游戏娱乐',$v[0]).'>游戏娱乐</option>
					<option value="硬件工具"'.$sel('硬件工具',$v[0]).'>硬件工具</option>
					<option value="手机数码"'.$sel('手机数码',$v[0]).'>手机数码</option>
					<option value="编程开发"'.$sel('编程开发',$v[0]).'>编程开发</option>
					<option value="管理软件"'.$sel('管理软件',$v[0]).'>管理软件</option>
					<option value="教育教学"'.$sel('教育教学',$v[0]).'>教育教学</option>
					<option value="其他"'.$sel('',$v[0]).'>其他</option>
				</select></td>'."\n";
	$s.='<td>软件名称及版本 <input type="text" name="cat08[]" value="'.$v[1].'" title="软件名称" class="w150" /></td>'."\n";
	$s.='<td>其他说明 <input type="text" name="cat08[]" value="'.$v[2].'" title="标题请不要超过一行" class="w150" /></td>'."\n";
	$s.='</table>'."\n";
	
	//cat09
	//学习 [学科名称][名称][格式][语言种类]
	$s.='<table  id="cat09" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="9001" >计算机</option>
					<option value="9002" >英语</option>
					<option value="9003" >考研</option>
					<option value="9004" >课件</option>
					<option value="9005" >其他</option>
					</select></td>'."\n";
	$s.='<td>名称 <input type="text" name="cat09[]" title="名称" class="w150" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>格式 <select name="cat09[]">
					<option value="MP3"'.$sel('MP3',$v[1]).'>MP3</option>
					<option value="WMV"'.$sel('WMV',$v[1]).'>WMV</option>
					<option value="RMVB"'.$sel('RMVB',$v[1]).'>RMVB</option>
					<option value="AVI"'.$sel('AVI',$v[1]).'>AVI</option>
					<option value="ISO"'.$sel('ISO',$v[1]).'>ISO</option>
					<option value="CHM"'.$sel('CHM',$v[1]).'>CHM</option>
					<option value="PDF"'.$sel('PDF',$v[1]).'>PDF</option>
					<option value="DOC"'.$sel('DOC',$v[1]).'>DOC</option>
					<option value="HTML"'.$sel('HTML',$v[1]).'>HTML</option>
					<option value="PPT"'.$sel('PPT',$v[1]).'>PPT</option>
					<option value="RAR"'.$sel('RAR',$v[1]).'>RAR</option>
					<option value="其他"'.$sel('',$v[1]).'>其他</option>
				</select></td>'."\n";
	$s.='<td>其他说明 <input type="text" name="cat09[]" title="标题请不要超过一行" class="w150" value="'.$v[2].'"  /></td>'."\n";
	$s.='</table>'."\n";
	

	//cat10 其他
	$s.='<table id="cat10" class="category" style="display:none">'."\n";
	$s.= '<td>类型   <select name="type[]">
					<option value="10001" >移动设备视频</option>
					<option value="10002" >图片</option>
					<option value="10003" >文档</option>
					</select></td>'."\n";
	$s.='<td><input type="text" name="cat10[]" title="名称" style="width:410px;" value="'.$v[0].'"  /></td>'."\n";
	$s.='</table>'."\n";

	
	
	//cat11纪录片
	$s.='<table id="cat11" class="category" style="display:none">';
	$s .= '<td>类型   <select name="type[]">
					<option value="11001" >国家地理</option>
					<option value="11003" >Discovery</option>
					<option value="11002" >IMAX</option>
					<option value="11004" >CCTV</option>
					<option value="11005" >BBC</option>
					<option value="11006" >其他</option>
					</select></td>'."\n";
	$s .='<td>年份   <input type="text" name="cat11[]" title="纪录片上映的年份" style="width:40px;" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>中文名 <input type="text" name="cat11[]" title="中文名称" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>英文名 <input type="text" name="cat11[]" title="英文名称" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>字幕   <select name="cat11[]">
					<option value="暂无字幕" '.$sel('暂无字幕',$v[5]).'>暂无字幕</option>
					<option value="英文字幕" '.$sel('英文字幕',$v[5]).'>英文字幕</option>
					<option value="中文字幕" '.$sel('中文字幕',$v[5]).'>中文字幕</option>
					<option value="中英字幕" '.$sel('中英字幕',$v[5]).'>中英字幕</option>
	...
	$s.='<td>格式   <select name="cat11[]" title="纪录片格式，具体含义请参看纪录片规则" >
					<option value="1080P"'.$sel('1080P',$v[6]).'>1080P</option>
					<option value="720P"'.$sel('720P',$v[6]).'>720P</option>
					<option value="HD-DVDRip"'.$sel('HD-DVDRip',$v[6]).'>HD-DVDRip</option>
					<option value="HR-HDTV"'.$sel('HR-HDTV',$v[6]).'>HR-HDTV</option>
	...
	//cat12高清
	$s.='<table id="cat12" class="category" style="display:none">';
	$s .= '<td>类型   <select name="type[]">
					<option value="12001" >电影</option>
					<option value="12002" >剧集</option>
					<option value="12003" >MV</option>
					<option value="12004" >纪录片</option>
					<option value="12005" >其他</option>
					</select></td>'."\n";
	$s .='<td>年份   <input type="text" name="cat12[]" title="上映年份" style="width:40px;" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>中文名 <input type="text" name="cat12[]" title="中文名称" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>英文名 <input type="text" name="cat12[]" title="英文名称" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>字幕   <select name="cat12[]">
					<option value="暂无字幕" '.$sel('暂无字幕',$v[5]).'>暂无字幕</option>
					<option value="英文字幕" '.$sel('英文字幕',$v[5]).'>英文字幕</option>
					<option value="中文字幕" '.$sel('中文字幕',$v[5]).'>中文字幕</option>
					<option value="中英字幕" '.$sel('中英字幕',$v[5]).'>中英字幕</option>
	...
	$s .='<td>高清类型   <input type="text" name="cat12[]" title="蓝光,原盘..." style="width:40px;" value="'.$v[0].'" /></td>'."\n";

	$s.='</table>'."\n";

	//cat13低质量
	$s.='<table id="cat13" class="category" style="display:none">';
	$s .='<td>类型    <select name="type[]">
					<option value="13001" >电影</option>
					<option value="13002" >剧集</option>
					<option value="13003" >其他</option>
			</select></td>'."\n";
	$s .='<td>年份	<input type="text" name="cat13[]" title="上映年份" style="width:40px;" value="'.$v[0].'" /></td>'."\n";
	$s.='<td>中文名	<input type="text" name="cat13[]" title="中文名称" value="'.$v[1].'" /></td>'."\n";
	$s.='<td>英文名	<input type="text" name="cat13[]" title="英文名称" value="'.$v[2].'" /></td>'."\n";
	$s.='<td>主演	<input type="text" name="cat13[]" title="主要演员" value="'.$v[3].'" /></td>'."\n";
	$s.='<td>字幕	<select name="cat13[]">
                                        <option value="暂无字幕" '.$sel('暂无字幕',$v[5]).'>暂无字幕</option>
                                        <option value="英文字幕" '.$sel('英文字幕',$v[5]).'>英文字幕</option>
                                        <option value="中文字幕" '.$sel('中文字幕',$v[5]).'>中文字幕</option>
                                        <option value="中英字幕" '.$sel('中英字幕',$v[5]).'>中英字幕</option>
	...
	$s.='<td>格式   <select name="cat13[]" >
					<option value="rm"'.$sel('rm',$v[6]).'>rm</option>
					<option value="rmvb"'.$sel('rmvb',$v[6]).'>rmvb</option>
					<option value="TS"'.$sel('TS',$v[6]).'>TS</option>
					<option value="avi"'.$sel('avi',$v[6]).'>avi</option>
					<option value="其他"'.$sel('',$v[6]).'>其他</option>
			</select></td>'."\n";
	$s.='</table>'."\n";

	//cat0 未选择分类提示
	$s.='<table id="cat0" class="category">'."\n";
	$s.='<td>请先选择大分类，然后根据提示填写命名表单</td>'."\n";
	$s.='</table>'."\n";
...
function selectOption($value,$sel){
...
//added by longth. FOR loginpt decode.  2011-01-08
