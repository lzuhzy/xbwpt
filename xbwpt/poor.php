...
dbconn(true);
require_once(get_langfile_path("poor.php"));
loggedinorreturn();
parked();
...
#########################################################added 20101229#####################################################################
						function newcat($listarry)
						{
							...
							{	
								{
								    if ($listarry[$i]['id'] == "13001" || $listarry[$i]['id'] == '13002' || $listarry[$i]['id'] == '13003') { //只显示低质量分类 Amanda add
									    $s .= "<div class=\"soncatname\"><a href=\"?cat=".$listarry[$i]['id']."\" ><b>".$listarry[$i]['name']."</b></a></div>";
								    }
								}
							}
							$s .="</div>";
