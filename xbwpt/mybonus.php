...
function bonusarray($option){
	global $onegbupload_bonus,$fivegbupload_bonus,$tengbupload_bonus,$fiftygbupload_bonus,$onehundredgbupload_bonus,$oneinvite_bonus,$customtitle_bonus,$vipstatus_bonus, $basictax_bonus, $taxpercentage_bonus, $bonusnoadpoint_advertisement, $bonusnoadtime_advertisement;
	global $lang_mybonus;
	$bonus = array();
	switch ($option)
	{
		...
		case 4: {//50.0 GB Uploaded
      			$bonus['points'] = $fiftygbupload_bonus;
		        $bonus['art'] = 'traffic';
    		...
    		case 5: {//100.0 GB Uploaded
      			$bonus['points'] = $onehundredgbupload_bonus;
      			$bonus['art'] = 'traffic';
      			$bonus['menge'] = 107374182400;
      			$bonus['name'] = $lang_mybonus['text_uploaded_five'];
      			$bonus['description'] = $lang_mybonus['text_uploaded_note'];
      			break;
      		}
		case 6: {//Invite
...
for ($i=1; $i <=11; $i++)
{
	...
	if (($i == 9 && $bonusgift_bonus == 'no') || ($i == 10 && ($enablead_advertisement == 'no' || $bonusnoad_advertisement == 'no')))
		continue;
	print("<tr>");
	...
	elseif ($i==9){  //for Give A Karma Gift
